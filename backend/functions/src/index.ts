...
const sendVerification = async (uid: string, recoveryEmail: string) => {
  ...
  await queueEmail(
    recoveryEmail,
    'Verify your eSmile parent email',
    `Verify the parent email for this account: ${link}\n\nThis link expires in 24 hours.`,
    `<h2>Verify the parent email</h2><p>Tap the button below to unlock the eSmile account.</p><p><a href="${link}" style="background:#6155F6;color:#fff;padding:12px 20px;border-radius:22px;text-decoration:none;font-weight:bold">Verify parent email</a></p><p>This link expires in 24 hours.</p>`
  );
};

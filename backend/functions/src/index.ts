...
const isValidUsernameInput = (value: string) => value.length >= 3 && value.length <= 64;
...
export const requestPasswordReset = onCall(callableOptions, async (request) => {
  ...
  if (!request.data || typeof request.data.username !== 'string' || typeof request.data.recoveryEmail !== 'string') return genericResult;
  if (!isValidUsernameInput(request.data.username)) return genericResult;

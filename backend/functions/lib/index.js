...
exports.aggregateGameSession = exports.aggregateBrushingSession = exports.requestPasswordReset = exports.verifyParentEmail = exports.getParentEmailVerificationStatus = exports.resendParentEmailVerification = exports.startParentEmailVerification = void 0;
...
const sendVerification = async (uid, recoveryEmail) => {
...
exports.startParentEmailVerification = (0, https_1.onCall)(callableOptions, async (request) => {
    ...
    if (!userDoc.exists || userDoc.data()?.role !== 'user') {
        throw new https_1.HttpsError('failed-precondition', 'The child account could not be found.');
    }
    await sendVerification(request.auth.uid, recoveryEmail);
    return { sent: true, recoveryEmailMasked: maskEmail(recoveryEmail) };
});
exports.resendParentEmailVerification = (0, https_1.onCall)(callableOptions, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Sign in is required.');
    const profile = await db.doc(`recoveryProfiles/${request.auth.uid}`).get();
    const data = profile.data();
    if (!profile.exists || !data?.recoveryEmail)
        throw new https_1.HttpsError('not-found', 'No recovery email is registered.');
    if (data.parentEmailVerified === true)
        return { sent: false, alreadyVerified: true };
    const lastSent = data.lastVerificationSentAt?.toMillis?.() ?? 0;
    if (Date.now() - lastSent < REQUEST_COOLDOWN_MS) {
        throw new https_1.HttpsError('resource-exhausted', 'Please wait one minute before requesting another email.');
    }
    await sendVerification(request.auth.uid, data.recoveryEmail);
    return { sent: true };
});
exports.getParentEmailVerificationStatus = (0, https_1.onCall)(callableOptions, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Sign in is required.');
...
exports.requestPasswordReset = (0, https_1.onCall)(callableOptions, async (request) => {
    ...
    if (!data || data.parentEmailVerified !== true || data.normalizedRecoveryEmail !== recoveryEmail)
        return genericResult;
...
exports.aggregateBrushingSession = (0, firestore_1.onDocumentCreated)('brushingSessions/{sessionId}', async (event) => {
...
exports.aggregateGameSession = (0, firestore_1.onDocumentCreated)('gameSessions/{sessionId}', async (event) => {
    const data = event.data?.data();
    if (!data?.gameId)
        return;

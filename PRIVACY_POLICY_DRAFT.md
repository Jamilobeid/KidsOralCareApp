...
The leaderboard remains an application feature. A child is listed only after the parent separately opts in through the approved leaderboard-consent process. It displays only the child’s chosen username, avatar, points, and level. It does not display the parent email, child age, detailed activity, location, or real name. A parent may withdraw leaderboard consent and request removal without deleting the entire account.
...
- Provide the child’s brushing timer, progress, reminders, games, rewards, personalization, and parent-approved leaderboard participation.

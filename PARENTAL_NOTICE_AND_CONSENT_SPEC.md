...
## 2. Leaderboard-preserving launch design
...
- Never display the parent email, child age, real name, detailed brushing/activity data, or location.
- Require the legally approved stronger consent method for leaderboard disclosure before setting `leaderboardDisclosureGranted` to true.
- Let children whose parents decline participation continue using every core brushing, game, reward, and parent-dashboard feature.
- Do not use advertising, behavioral analytics, location, camera, microphone recording, or social/chat features.

This preserves the leaderboard without pretending that public disclosure is covered by an internal-use email-plus consent.
...
2. **Optional leaderboard disclosure** — a separate, unselected choice authorizing publication of only the child’s chosen username, avatar, points, and level after the stronger approved verification method succeeds.
...
- `leaderboardDisclosureGranted`: boolean; false unless the parent separately opts in and the stronger approved verification succeeds.
...
- A child whose separate leaderboard consent is false is not listed, while the leaderboard feature and all other application functionality remain available.
...
3. The legally approved stronger verification method for optional leaderboard disclosure.

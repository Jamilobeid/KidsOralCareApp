...
const SugarDetectiveGame = ({ canPlayAgain, onReplay, onStart, onComplete }: { canPlayAgain: boolean; onReplay: () => boolean; onStart: () => boolean; onComplete: () => void }) => {
  ...
  return (
    <Card style={[styles.gamePanelBorder, styles.sugarCard]}>
      ...
      <Text style={[buttonFont, styles.sugarQuestion, { color: theme.text }]}>{t('sugarQuestion')}</Text>
        ...
        {options.map((choice) => {
          ...
          return (
            ...
            >
              ...
              <Text style={[buttonFont, styles.sugarOptionLabel, { color: theme.text }]}>{t('cubes')}</Text>
            </Pressable>
          );
        })}
      </View>
...
const CleanToothGame = ({ cleaned, canPlayAgain, onReplay, onClean, onReset, onComplete }: { cleaned: boolean[]; canPlayAgain: boolean; onReplay: () => boolean; onClean: (index: number) => void; onReset: () => void; onComplete: () => void }) => {
  ...
  return (
    <Card style={[styles.gamePanelBorder, styles.cleanCard, styles.cleanPlayCard]}>
      <Text style={[buttonFont, styles.cleanSubtitle, { color: '#4E7771' }]}>{t('cleanSubtitle')}</Text>
...
const styles = StyleSheet.create({
  ...
  toothChoiceGrid: { width: '100%', flexDirection: 'row', flexWrap: 'wrap', gap: 14, justifyContent: 'center' },
  toothChoice: { width: '43%', maxWidth: 142, height: 146, backgroundColor: '#FFFFFF', borderRadius: 20, alignItems: 'center', justifyContent: 'center', padding: 10, shadowColor: '#17324D', shadowOpacity: 0.08, shadowRadius: 7, shadowOffset: { width: 0, height: 4 }, elevation: 10 },
